...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.savingAccount.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.saving-accounts.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="bank_id">{{ trans('cruds.savingAccount.fields.bank_id') }}</label>
                <input class="form-control {{ $errors->has('bank_id') ? 'is-invalid' : '' }}" type="text" name="bank_id" id="bank_id" value="{{ old('bank_id', '') }}">
                @if($errors->has('bank_id'))
                    <div class="invalid-feedback">
                        {{ $errors->first('bank_id') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.savingAccount.fields.bank_id_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="bank_name">{{ trans('cruds.savingAccount.fields.bank_name') }}</label>
            ...
            <div class="form-group">
                <label class="required" for="bank_account_name">{{ trans('cruds.savingAccount.fields.bank_account_name') }}</label>
            ...
            <div class="form-group">
                <label class="required" for="bank_account_number">{{ trans('cruds.savingAccount.fields.bank_account_number') }}</label>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('currency') ? 'is-invalid' : '' }}" name="currency_id" id="currency_id">
                    ...
                    @foreach($currencies as $id => $currency)
                        <option value="{{ $currency['id'] }}" >{{ $currency['name'] }}&nbsp;-&nbsp;{{ $currency['short_code'] }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="daily_limit">{{ trans('cruds.savingAccount.fields.daily_limit') }}</label>
                <input class="form-control {{ $errors->has('daily_limit') ? 'is-invalid' : '' }}" type="number" name="daily_limit" id="daily_limit" value="{{ old('daily_limit', '') }}" step="0.01">
            ...
            <div class="form-group">
                <label class="required" for="transaction_limit">{{ trans('cruds.savingAccount.fields.transaction_limit') }}</label>
                <input class="form-control {{ $errors->has('transaction_limit') ? 'is-invalid' : '' }}" type="number" name="transaction_limit" id="transaction_limit" value="{{ old('transaction_limit', '') }}" step="0.01">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.savingAccount.fields.status') }}</label>
