    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.partners.update", [$partner->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $partner->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <label class="required" for="password">{{ trans('cruds.partner.fields.password') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.partner.fields.password_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="roles">{{ trans('cruds.partner.fields.roles') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" required>
                    @foreach($roles as $id => $roles)
                        <option value="{{ $id }}" {{ (in_array($id, old('roles', [])) || $partner->roles->contains($id)) ? 'selected' : '' }}>{{ $roles }}</option>
                    @endforeach
                </select>
                @if($errors->has('roles'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.partner.fields.roles_helper') }}</span>
            </div>
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Partner::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $partner->status) === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
