    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.balances.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('document_no') ? 'is-invalid' : '' }}" type="text" name="document_no" id="document_no" value="{{ old('document_no', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('settlement_status') ? 'is-invalid' : '' }}" name="settlement_status" id="settlement_status">
                    ...
                    @foreach(App\Models\Balance::SETTLEMENT_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('settlement_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bank') ? 'is-invalid' : '' }}" name="settlement_bank_id" id="settlement_bank_id">
                    @foreach($settlement_banks as $id => $bank)
                        <option value="{{ $id }}" {{ old('settlement_bank_id') == $id ? 'selected' : '' }}>{{ $bank }}</option>
                    @endforeach
                </select>
                @if($errors->has('bank'))
                    <div class="invalid-feedback">
