...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.payoutBank.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.payout-banks.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bank_currency') ? 'is-invalid' : '' }}" type="text" name="bank_currency" id="bank_currency" value="{{ old('bank_currency', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payout') ? 'is-invalid' : '' }}" name="payout_id" id="payout_id">
                    @foreach($payouts as $id => $payout)
                        <option value="{{ $id }}" {{ old('payout_id') == $id ? 'selected' : '' }}>{{ $payout }}</option>
                    @endforeach
                </select>
