    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.merchants.update", [$merchant->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $merchant->email) }}">
                @if($errors->has('email'))
                    <div class="invalid-feedback">
                        {{ $errors->first('email') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('person_incharge_name') ? 'is-invalid' : '' }}" type="text" name="person_incharge_name" id="person_incharge_name" value="{{ old('person_incharge_name', $merchant->person_incharge_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contact') ? 'is-invalid' : '' }}" type="number" name="contact" id="contact" value="{{ old('contact', $merchant->contact) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" required>
                    @foreach($roles as $id => $roles)
                        <option value="{{ $id }}" {{ (in_array($id, old('roles', [])) || $merchant->roles->contains($id)) ? 'selected' : '' }}>{{ $roles }}</option>
                    @endforeach
                </select>
                @if($errors->has('roles'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\merchant::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $merchant->status) === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
